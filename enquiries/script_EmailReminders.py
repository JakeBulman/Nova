...
for task in TaskManager.objects.filter(task_id='RETMIS', task_completion_date__isnull=True):
    script = task.ec_sid
    examiner = ScriptApportionment.objects.get(ec_sid = script).enpe_sid.per_sid.exm_creditor_no
...
for exm in examiner_list:
    ...
    for enpe in EnquiryPersonnelDetails.objects.filter(exm_creditor_no=exm):
        for script in ScriptApportionment.objects.filter(enpe_sid=enpe.enpe_sid):
            task = TaskManager.objects.get(task_id='RETMIS', task_completion_date__isnull=True, ec_sid=script.ec_sid.ec_sid)
            extension_total = 0
