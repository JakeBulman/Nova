...
def load_core_tables():
    ...
    EarServerSettings.objects.update(delta_load_status='Secondary Load Starting, first table loading')
    ...
    try:
        ...
        EarServerSettings.objects.update(delta_load_status='Secondary - Table 1 of 4 loaded, Examiner Panels')
        ...
        EarServerSettings.objects.update(delta_load_status='Secondary - Table 2 of 4 loaded, Unique Creditors')
        ...
        EarServerSettings.objects.update(delta_load_status='Secondary - Table 3 of 4 loaded, Enquiry Personnel')
        ...
        EarServerSettings.objects.update(delta_load_status='Secondary - Table 4 of 4 loaded, Enquiry Component Examiner Checks')


        if os.getenv('DJANGO_PRODUCTION') == 'true':
            EarServerSettings.objects.update(delta_load_status='Secondary Load completed at '+str(datetime.datetime.now().strftime("%d/%m/%Y, %H:%M:%S")))
            email = EmailMessage()
            ...
            email["Subject"] = "Secondary Data Load - Secondary - SUCCESS"
            email.set_content("Secondary load was successful, Panel data updated.", subtype='html')
        ...
        if os.getenv('DJANGO_PRODUCTION') == 'true':
            EarServerSettings.objects.update(delta_load_status='Secondary Load failed at '+str(datetime.datetime.now().strftime("%d/%m/%Y, %H:%M:%S")))
            email = EmailMessage()
            email["From"] = "results.enquiries@cambridge.org"
            email["To"] = "results.enquiries@cambridge.org, jacob.bulman@cambridge.org,jonathon.east@cambridge.org,ben.herbert@cambridge.org,charlotte.weedon@cambridge.org,morgan.jones@cambridge.org,lab.d@cambridgeassessment.org.uk"
            email["Subject"] = "Secondary Data Load - ERROR"
            email.set_content(f"Secondary load has failed, please contact the system administrator for further details. Error: {e}", subtype='html')
