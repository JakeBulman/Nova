...
from enquiries.models import CentreEnquiryRequests, EnquiryRequestParts, EnquiryComponents, EnquiryPersonnel, EnquiryPersonnelDetails, EnquiryBatches, EnquiryComponentElements, TaskManager, UniqueCreditor, EnquiryComponentsHistory, EnquiryComponentsExaminerChecks, TaskTypes, EarServerSettings
...
def load_core_tables():
    ...
    def insert_to_model_enpe(row):
        if UniqueCreditor.objects.filter(exm_creditor_no = row['exm_creditor_no']).exists():
    ...
    def insert_to_model_enpe(row):
        if EnquiryPersonnel.objects.filter(enpe_sid = row['enpe_sid']).exists():
            EnquiryPersonnel.objects.filter(enpe_sid = row['enpe_sid']).update(
