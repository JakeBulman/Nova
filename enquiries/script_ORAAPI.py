...
for item in APIMAR_data:
    ...
    print(item.eps_cand_id)
    print(item.final_mark)
    if item.final_mark == 'None':
        input_mark = None
    else:
         input_mark: item.final_mark
    try :
        cursor.execute("""
    ...
    """,{
        ...
        cursor.execute("""
        INSERT INTO ETL_PRD.EAR_REMARK_STAGING (eb_sid, centre, cand_no, mark_confirmed_ind, mark_for_input, justification_code, enquiry_status, examiner_mark) VALUES (:eb_sid, :centre, :cand_no, :mark_confirmed_ind, :mark_for_input, :justification_code, :enquiry_status, :examiner_mark)
    """,{
    
        'eb_sid': item.eb_sid.eb_sid,
        'centre': item.eps_centre_id,
        'cand_no': item.eps_cand_id,
        'mark_confirmed_ind': item.mark_confirmed_ind,
        'mark_for_input':input_mark,
        'justification_code': item.selected_justification_code,
        'enquiry_status':'O',
        'examiner_mark': item.scaled_mark
    })
        DjangoStagingTableAPP.objects.filter(pk=item.pk).update(copied_to_est=1)
        oracle_conn.commit()
        print("Data Transfer has been completed")
