...
filename=os.path.join("Y:\Operations\Change Delivery Team\Project Documents\\Nova\EAR Test Data\\N23 Examiner Unavail.xlsx")
...
for row in sheet.iter_rows():
    try:
        creditor = str(row[7].value)
        print(creditor)
        start = str(row[1].value).split(' ')[0]
        print(start)
        end = str(row[2].value).split(' ')[0]
        print(end)

        exm = UniqueCreditor.objects.get(exm_creditor_no=creditor)
        print(exm)
        avail = ExaminerAvailability(
        unavailability_start_date = start,
        unavailability_end_date = end,
